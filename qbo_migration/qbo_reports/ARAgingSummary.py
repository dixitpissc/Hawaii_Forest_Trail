...
REPORT_TABLE  = os.getenv("ARAgingSummary_TABLE", "ARAgingSummary")
SAVE_TO_SQL   = os.getenv("ARAgingSummary_SAVE_TO_SQL", "true").lower() == "true"

# Report options (only sent when provided)
AS_OF_DATE    = os.getenv("ARAgingSummary_AS_OF_DATE")               # yyyy-mm-dd
DATE_MACRO    = os.getenv("ARAgingSummary_DATE_MACRO")               # e.g. "This Fiscal Year-to-date"
CUSTOMER_IDS  = os.getenv("ARAgingSummary_CUSTOMER_IDS")             # "123,456"
ACCOUNTING    = os.getenv("ARAgingSummary_ACCOUNTING_METHOD")        # "Cash"|"Accrual"
CURRENCY      = os.getenv("ARAgingSummary_CURRENCY")                 # "USD"|"CAD"|...
MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (if ignored by report, QBO safely discards)
START_POS     = int(os.getenv("ARAgingSummary_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("ARAgingSummary_PAGESIZE", "1000"))
...
REPORT_NAME = "AgedReceivables"  # ← A/R Aging Summary
...
def _extract_columns_meta(report: dict):
    ...
    for i, c in enumerate(cols):
        ...
        out_columns.append(f"{base}__Id")      # id (often empty but kept)
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching A/R Aging Summary from QBO")
    try:
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("ARAgingSummary_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ ARAgingSummary: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_ar_aging_summary():
    logger.info("🚀 Starting A/R Aging Summary report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (ARAgingSummary_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ ARAgingSummary report failed: {msg[:2000]}")
        raise
