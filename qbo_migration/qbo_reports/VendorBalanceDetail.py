...
REPORT_TABLE  = os.getenv("VendorBalanceDetail_TABLE", "VendorBalanceDetail")
SAVE_TO_SQL   = os.getenv("VendorBalanceDetail_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("VendorBalanceDetail_START_DATE")               # yyyy-mm-dd
END_DATE      = os.getenv("VendorBalanceDetail_END_DATE")                 # yyyy-mm-dd
DATE_MACRO    = os.getenv("VendorBalanceDetail_DATE_MACRO")               # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("VendorBalanceDetail_ACCOUNTING_METHOD")        # "Cash"|"Accrual"
CURRENCY      = os.getenv("VendorBalanceDetail_CURRENCY")                 # "USD"|"CAD"|...
COLUMNS_BY    = os.getenv("VendorBalanceDetail_COLUMNS")                  # e.g., "Total" or "Months"
SUMMARIZE_BY  = os.getenv("VendorBalanceDetail_SUMMARIZE_COLUMN_BY")      # legacy alias, if you use it

# Filters (QBO ignores unrecognized gracefully)
VENDOR_IDS    = os.getenv("VendorBalanceDetail_VENDOR_IDS")               # CSV
ACCOUNT_IDS   = os.getenv("VendorBalanceDetail_ACCOUNT_IDS")              # CSV
CUSTOMER_IDS  = os.getenv("VendorBalanceDetail_CUSTOMER_IDS")             # CSV
EMPLOYEE_IDS  = os.getenv("VendorBalanceDetail_EMPLOYEE_IDS")             # CSV
CLASS_IDS     = os.getenv("VendorBalanceDetail_CLASS_IDS")                # CSV
DEPT_IDS      = os.getenv("VendorBalanceDetail_DEPARTMENT_IDS")           # CSV
PAYMENTMETHOD_IDS = os.getenv("VendorBalanceDetail_PAYMENT_METHOD_IDS")   # CSV
NAME_IDS      = os.getenv("VendorBalanceDetail_NAME_IDS")                 # CSV (generic Name filter)
TXN_TYPE      = os.getenv("VendorBalanceDetail_TRANSACTION_TYPE")         # e.g., "Bill,BillPaymentCheck,Check" (CSV)

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; harmless if so)
START_POS     = int(os.getenv("VendorBalanceDetail_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("VendorBalanceDetail_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("VendorBalanceDetail_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("VendorBalanceDetail_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Vendor Balance Detail from QBO")
    try:
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("VendorBalanceDetail_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ VendorBalanceDetail: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_vendor_balance_detail():
    logger.info("🚀 Starting Vendor Balance Detail report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (VendorBalanceDetail_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ VendorBalanceDetail report failed: {msg[:2000]}")
        raise
...
# VendorBalanceDetail_SAVE_TO_SQL=true
# VendorBalanceDetail_SAVE_EMPTY_SCHEMA=true

# Date range or macro
# VendorBalanceDetail_START_DATE=2024-01-01
# VendorBalanceDetail_END_DATE=2024-12-31
# # or
# # VendorBalanceDetail_DATE_MACRO=This Fiscal Year-to-date

# # Columns / breakdown
# VendorBalanceDetail_COLUMNS=Total
# # or: VendorBalanceDetail_SUMMARIZE_COLUMN_BY=Months

# # Filters (optional)
# VendorBalanceDetail_ACCOUNTING_METHOD=Accrual
# VendorBalanceDetail_CURRENCY=USD
# VendorBalanceDetail_VENDOR_IDS=100,101,102
# VendorBalanceDetail_ACCOUNT_IDS=1,2,3
# VendorBalanceDetail_CUSTOMER_IDS=10,11
# VendorBalanceDetail_EMPLOYEE_IDS=30,31
# VendorBalanceDetail_CLASS_IDS=40,41
# VendorBalanceDetail_DEPARTMENT_IDS=50,51
# VendorBalanceDetail_PAYMENT_METHOD_IDS=60,61
# VendorBalanceDetail_NAME_IDS=70,71
# VendorBalanceDetail_TRANSACTION_TYPE=Bill,BillPaymentCheck,Check,Expense,JournalEntry

# # Paging & insert behavior
# VendorBalanceDetail_STARTPOSITION=1
# VendorBalanceDetail_PAGESIZE=2000
# VendorBalanceDetail_FORCE_TEXT_INSERT=false
# VendorBalanceDetail_COERCE_TABLE_TO_TEXT=true
