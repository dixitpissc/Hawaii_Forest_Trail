...
REPORT_TABLE  = os.getenv("GeneralLedger_TABLE", "GeneralLedger")
SAVE_TO_SQL   = os.getenv("GeneralLedger_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("GeneralLedger_START_DATE")               # yyyy-mm-dd
END_DATE      = os.getenv("GeneralLedger_END_DATE")                 # yyyy-mm-dd
DATE_MACRO    = os.getenv("GeneralLedger_DATE_MACRO")               # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("GeneralLedger_ACCOUNTING_METHOD")        # "Cash"|"Accrual"
CURRENCY      = os.getenv("GeneralLedger_CURRENCY")                 # "USD"|"CAD"|...
ACCOUNT_IDS   = os.getenv("GeneralLedger_ACCOUNT_IDS")              # "10,12,15" (QBO Account IDs)
TXN_TYPES     = os.getenv("GeneralLedger_TRANSACTION_TYPES")        # CSV of txn types, if you use this filter
POSTING_TYPE  = os.getenv("GeneralLedger_POSTING_TYPE")             # "Posting"|"NonPosting" (if supported)
COLUMNS_BY    = os.getenv("GeneralLedger_COLUMNS")                  # request specific columns (optional; can hide rows if misused)
MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; safe to include)
START_POS     = int(os.getenv("GeneralLedger_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("GeneralLedger_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("GeneralLedger_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("GeneralLedger_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
def _build_params() -> dict:
    ...
    if ACCOUNT_IDS:  p["account"]           = ACCOUNT_IDS  # CSV of Account Ids
    if TXN_TYPES:    p["transaction_type"]  = TXN_TYPES    # CSV of types
    if POSTING_TYPE: p["posting_type"]      = POSTING_TYPE
    if COLUMNS_BY:   p["columns"]           = COLUMNS_BY   # optional — use carefully
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching General Ledger from QBO")
    try:
    ...
    # Optional paging loop (enable if tenant returns more than one page)
    # frames = [df]
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("GeneralLedger_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ GeneralLedger: Nothing to save (empty DataFrame).")
        return
...
def run_general_ledger():
    logger.info("🚀 Starting General Ledger report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (GeneralLedger_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ GeneralLedger report failed: {msg[:2000]}")
        raise


# Env variable
# GeneralLedger_SAVE_TO_SQL=true
# GeneralLedger_SAVE_EMPTY_SCHEMA=true

# # Date range or macro
# GeneralLedger_START_DATE=2010-01-01
# GeneralLedger_END_DATE=2025-09-02
# # or:
# # GeneralLedger_DATE_MACRO=This Fiscal Year-to-date

# # Filters (optional — provide only if you need them)
# GeneralLedger_ACCOUNTING_METHOD=Accrual
# GeneralLedger_CURRENCY=USD
# GeneralLedger_ACCOUNT_IDS=10,11,21
# GeneralLedger_TRANSACTION_TYPES=JournalEntry,Invoice,Bill,Payment
# GeneralLedger_POSTING_TYPE=Posting
# # Request explicit columns only if you know you need them:
# # GeneralLedger_COLUMNS=tx_date,txn_type,doc_num,account_name,subt_nat_amount

# # Paging
# GeneralLedger_STARTPOSITION=1
# GeneralLedger_PAGESIZE=2000

# # Insert behavior
# GeneralLedger_FORCE_TEXT_INSERT=false
# GeneralLedger_COERCE_TABLE_TO_TEXT=true
