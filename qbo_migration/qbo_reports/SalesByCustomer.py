...
REPORT_TABLE  = os.getenv("SalesByCustomer_TABLE", "SalesByCustomer")
SAVE_TO_SQL   = os.getenv("SalesByCustomer_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("SalesByCustomer_START_DATE")               # yyyy-mm-dd
END_DATE      = os.getenv("SalesByCustomer_END_DATE")                 # yyyy-mm-dd
DATE_MACRO    = os.getenv("SalesByCustomer_DATE_MACRO")               # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("SalesByCustomer_ACCOUNTING_METHOD")        # "Cash"|"Accrual"
CURRENCY      = os.getenv("SalesByCustomer_CURRENCY")                 # "USD"|"CAD"|...

# Column breakdowns (Months, Total, etc.)
COLUMNS_BY    = os.getenv("SalesByCustomer_COLUMNS")                  # e.g., "Months" or "Total"
SUMMARIZE_BY  = os.getenv("SalesByCustomer_SUMMARIZE_COLUMN_BY")      # legacy alias if you use it

# Filters
CUSTOMER_IDS  = os.getenv("SalesByCustomer_CUSTOMER_IDS")             # CSV
ITEM_IDS      = os.getenv("SalesByCustomer_ITEM_IDS")                 # CSV
CLASS_IDS     = os.getenv("SalesByCustomer_CLASS_IDS")                # CSV
DEPT_IDS      = os.getenv("SalesByCustomer_DEPARTMENT_IDS")           # CSV
NAME_IDS      = os.getenv("SalesByCustomer_NAME_IDS")                 # CSV (generic Name filter)

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; safe to include)
START_POS     = int(os.getenv("SalesByCustomer_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("SalesByCustomer_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("SalesByCustomer_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("SalesByCustomer_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
REPORT_NAME = "CustomerSales"  # Sales by Customer (Summary)
...
def _extract_columns_meta(report: dict):
    ...
    Return meta + names like:
      [{"idx":0,"title":"Customer","colkey":"cust_name","safe":"Customer__cust_name"}, ...]
    and output columns (value + Id):
      ["Customer__cust_name", "Customer__cust_name__Id", ...]
    """
...
def _walk_rows_dynamic(rows_container: dict, col_meta: list, current_group_path: list, rows_out: list):
    ...
    for r in rows:
        ...
        if rtype == "Section":
            ...
            # Optional section summary (per-customer totals)
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Sales by Customer from QBO")
    try:
    ...
    # Optional multipage loop pattern (rare for this report)
    return df
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("SalesByCustomer_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ SalesByCustomer: Nothing to save (empty DataFrame).")
        return
...
def run_sales_by_customer():
    logger.info("🚀 Starting Sales by Customer report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (SalesByCustomer_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ SalesByCustomer report failed: {msg[:2000]}")
        raise
...
# SalesByCustomer_SAVE_TO_SQL=true
# SalesByCustomer_SAVE_EMPTY_SCHEMA=true

# # Date range or macro (summary is range-based)
# SalesByCustomer_START_DATE=2024-01-01
# SalesByCustomer_END_DATE=2024-12-31
# # or
# # SalesByCustomer_DATE_MACRO=This Fiscal Year-to-date

# # Columns / breakdown (e.g., Months or Total)
# SalesByCustomer_COLUMNS=Months
# # or: SalesByCustomer_SUMMARIZE_COLUMN_BY=Months

# # Filters (optional)
# SalesByCustomer_ACCOUNTING_METHOD=Accrual
# SalesByCustomer_CURRENCY=USD
# SalesByCustomer_CUSTOMER_IDS=123,456,789
# SalesByCustomer_ITEM_IDS=200,201
# SalesByCustomer_CLASS_IDS=10,11
# SalesByCustomer_DEPARTMENT_IDS=1,2
# SalesByCustomer_NAME_IDS=888,889

# # Paging & insert behavior
# SalesByCustomer_STARTPOSITION=1
# SalesByCustomer_PAGESIZE=2000
# SalesByCustomer_FORCE_TEXT_INSERT=false
# SalesByCustomer_COERCE_TABLE_TO_TEXT=true
