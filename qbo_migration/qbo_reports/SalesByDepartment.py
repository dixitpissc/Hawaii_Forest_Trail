...
REPORT_TABLE  = os.getenv("SalesByDepartment_TABLE", "SalesByDepartment")
SAVE_TO_SQL   = os.getenv("SalesByDepartment_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("SalesByDepartment_START_DATE")               # yyyy-mm-dd
END_DATE      = os.getenv("SalesByDepartment_END_DATE")                 # yyyy-mm-dd
DATE_MACRO    = os.getenv("SalesByDepartment_DATE_MACRO")               # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("SalesByDepartment_ACCOUNTING_METHOD")        # "Cash"|"Accrual"
CURRENCY      = os.getenv("SalesByDepartment_CURRENCY")                 # "USD"|"CAD"|...

# Column breakdowns (Months, Total, etc.)
COLUMNS_BY    = os.getenv("SalesByDepartment_COLUMNS")                  # e.g., "Months"
SUMMARIZE_BY  = os.getenv("SalesByDepartment_SUMMARIZE_COLUMN_BY")      # legacy alias

# Filters (QBO ignores unknowns gracefully)
DEPT_IDS      = os.getenv("SalesByDepartment_DEPARTMENT_IDS")           # CSV of Department IDs
CUSTOMER_IDS  = os.getenv("SalesByDepartment_CUSTOMER_IDS")             # CSV
ITEM_IDS      = os.getenv("SalesByDepartment_ITEM_IDS")                 # CSV
CLASS_IDS     = os.getenv("SalesByDepartment_CLASS_IDS")                # CSV
NAME_IDS      = os.getenv("SalesByDepartment_NAME_IDS")                 # CSV (generic Name filter)

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; safe to include)
START_POS     = int(os.getenv("SalesByDepartment_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("SalesByDepartment_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("SalesByDepartment_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("SalesByDepartment_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
REPORT_NAME = "DepartmentSales"  # Sales by Department (Summary) — Intuit SDK class name. :contentReference[oaicite:1]{index=1}
...
def _extract_columns_meta(report: dict):
    ...
    Return meta + names like:
      [{"idx":0,"title":"Department","colkey":"dept_name","safe":"Department__dept_name"}, ...]
    and output columns (value + Id):
      ["Department__dept_name", "Department__dept_name__Id", ...]
    """
...
def _walk_rows_dynamic(rows_container: dict, col_meta: list, current_group_path: list, rows_out: list):
    ...
    for r in rows:
        ...
        if rtype == "Section":
            ...
            # Optional section summary (per-department totals)
...
def _append_top_level_summary(report: dict, col_meta: list, rows_out: list):
    """Capture Rows.Summary (grand totals across departments) as RowType='ReportSummary' when present."""
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Sales by Department from QBO")
    try:
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("SalesByDepartment_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ SalesByDepartment: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_sales_by_department():
    logger.info("🚀 Starting Sales by Department report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (SalesByDepartment_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ SalesByDepartment report failed: {msg[:2000]}")
        raise
...
# Env Credential
# SalesByDepartment_SAVE_TO_SQL=true
# SalesByDepartment_SAVE_EMPTY_SCHEMA=true

# # Date range or macro
# SalesByDepartment_START_DATE=2024-01-01
# SalesByDepartment_END_DATE=2024-12-31
# # or
# # SalesByDepartment_DATE_MACRO=This Fiscal Year-to-date

# # Columns / breakdown (e.g., Months or Total)
# SalesByDepartment_COLUMNS=Months
# # or: SalesByDepartment_SUMMARIZE_COLUMN_BY=Months

# # Filters (optional)
# SalesByDepartment_ACCOUNTING_METHOD=Accrual
# SalesByDepartment_CURRENCY=USD
# SalesByDepartment_DEPARTMENT_IDS=1,2,3
# SalesByDepartment_CUSTOMER_IDS=123,456
# SalesByDepartment_ITEM_IDS=200,201
# SalesByDepartment_CLASS_IDS=10,11
# SalesByDepartment_NAME_IDS=888,889

# # Paging & insert behavior
# SalesByDepartment_STARTPOSITION=1
# SalesByDepartment_PAGESIZE=2000
# SalesByDepartment_FORCE_TEXT_INSERT=false
# SalesByDepartment_COERCE_TABLE_TO_TEXT=true
