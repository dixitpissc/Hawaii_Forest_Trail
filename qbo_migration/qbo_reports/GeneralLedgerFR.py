...
REPORT_TABLE  = os.getenv("GeneralLedgerFR_TABLE", "GeneralLedgerFR")
SAVE_TO_SQL   = os.getenv("GeneralLedgerFR_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("GeneralLedgerFR_START_DATE")            # yyyy-mm-dd
END_DATE      = os.getenv("GeneralLedgerFR_END_DATE")              # yyyy-mm-dd
DATE_MACRO    = os.getenv("GeneralLedgerFR_DATE_MACRO")            # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("GeneralLedgerFR_ACCOUNTING_METHOD")     # "Cash"|"Accrual"
CURRENCY      = os.getenv("GeneralLedgerFR_CURRENCY")              # "USD"|"CAD"|...
ACCOUNT_IDS   = os.getenv("GeneralLedgerFR_ACCOUNT_IDS")           # CSV of QBO Account Ids
TXN_TYPES     = os.getenv("GeneralLedgerFR_TRANSACTION_TYPES")     # CSV: JournalEntry,Invoice,...
POSTING_TYPE  = os.getenv("GeneralLedgerFR_POSTING_TYPE")          # "Posting"|"NonPosting"
COLUMNS_BY    = os.getenv("GeneralLedgerFR_COLUMNS")               # optional explicit cols
JOURNAL_CODES = os.getenv("GeneralLedgerFR_JOURNAL_CODES")         # optional, if your locale supports it
MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; safe to include)
START_POS     = int(os.getenv("GeneralLedgerFR_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("GeneralLedgerFR_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("GeneralLedgerFR_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("GeneralLedgerFR_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
def _build_params() -> dict:
    ...
    if JOURNAL_CODES: p["journal_code"]      = JOURNAL_CODES  # optional, if supported by your locale/edition
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching General Ledger FR from QBO")
    try:
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("GeneralLedgerFR_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ GeneralLedgerFR: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_general_ledger_fr():
    logger.info("🚀 Starting General Ledger FR report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (GeneralLedgerFR_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ GeneralLedgerFR report failed: {msg[:2000]}")
        raise
...
# Env details
# GeneralLedgerFR_SAVE_TO_SQL=true
# GeneralLedgerFR_SAVE_EMPTY_SCHEMA=true

# # Date range or macro
# GeneralLedgerFR_START_DATE=2024-01-01
# GeneralLedgerFR_END_DATE=2024-12-31
# # or:
# # GeneralLedgerFR_DATE_MACRO=This Fiscal Year-to-date

# # Filters (optional)
# GeneralLedgerFR_ACCOUNTING_METHOD=Accrual
# GeneralLedgerFR_CURRENCY=EUR
# GeneralLedgerFR_ACCOUNT_IDS=10,11,21
# GeneralLedgerFR_TRANSACTION_TYPES=JournalEntry,Invoice,Bill,Payment
# GeneralLedgerFR_POSTING_TYPE=Posting
# # optional/locale-specific — only if your tenant supports it:
# GeneralLedgerFR_JOURNAL_CODES=AC,VE,OD

# # Paging
# GeneralLedgerFR_STARTPOSITION=1
# GeneralLedgerFR_PAGESIZE=2000

# # Insert behavior
# GeneralLedgerFR_FORCE_TEXT_INSERT=false
# GeneralLedgerFR_COERCE_TABLE_TO_TEXT=true
