...
REPORT_TABLE  = os.getenv("TransactionListByVendor_TABLE", "TransactionListByVendor")
SAVE_TO_SQL   = os.getenv("TransactionListByVendor_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("TransactionListByVendor_START_DATE")               # yyyy-mm-dd
END_DATE      = os.getenv("TransactionListByVendor_END_DATE")                 # yyyy-mm-dd
DATE_MACRO    = os.getenv("TransactionListByVendor_DATE_MACRO")               # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("TransactionListByVendor_ACCOUNTING_METHOD")        # "Cash"|"Accrual"
CURRENCY      = os.getenv("TransactionListByVendor_CURRENCY")                 # "USD"|"CAD"|...
COLUMNS_BY    = os.getenv("TransactionListByVendor_COLUMNS")                  # e.g., "Total" or "Months"
SUMMARIZE_BY  = os.getenv("TransactionListByVendor_SUMMARIZE_COLUMN_BY")      # legacy alias, if you use it

# Filters (QBO ignores unrecognized gracefully)
VENDOR_IDS    = os.getenv("TransactionListByVendor_VENDOR_IDS")               # CSV
ACCOUNT_IDS   = os.getenv("TransactionListByVendor_ACCOUNT_IDS")              # CSV
CUSTOMER_IDS  = os.getenv("TransactionListByVendor_CUSTOMER_IDS")             # CSV
EMPLOYEE_IDS  = os.getenv("TransactionListByVendor_EMPLOYEE_IDS")             # CSV
CLASS_IDS     = os.getenv("TransactionListByVendor_CLASS_IDS")                # CSV
DEPT_IDS      = os.getenv("TransactionListByVendor_DEPARTMENT_IDS")           # CSV
PAYMENTMETHOD_IDS = os.getenv("TransactionListByVendor_PAYMENT_METHOD_IDS")   # CSV
NAME_IDS      = os.getenv("TransactionListByVendor_NAME_IDS")                 # CSV (generic Name filter)
TXN_TYPE      = os.getenv("TransactionListByVendor_TRANSACTION_TYPE")         # e.g., "Bill,BillPaymentCheck,Check" (comma sep)

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; harmless if so)
START_POS     = int(os.getenv("TransactionListByVendor_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("TransactionListByVendor_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("TransactionListByVendor_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("TransactionListByVendor_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
REPORT_NAME = "TransactionListByVendor"
...
def _walk_rows_dynamic(rows_container: dict, col_meta: list, current_group_path: list, rows_out: list):
    ...
    for r in rows:
        ...
        if rtype == "Section":
            ...
            # Optional per-section summary (e.g., per vendor)
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Transaction List by Vendor from QBO")
    try:
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("TransactionListByVendor_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ TransactionListByVendor: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_transaction_list_by_vendor():
    logger.info("🚀 Starting Transaction List by Vendor report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (TransactionListByVendor_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ TransactionListByVendor report failed: {msg[:2000]}")
        raise
...
# Env Credential
# TransactionListByVendor_SAVE_TO_SQL=true
# TransactionListByVendor_SAVE_EMPTY_SCHEMA=true

# # Date range or macro
# TransactionListByVendor_START_DATE=2024-01-01
# TransactionListByVendor_END_DATE=2024-12-31
# # or
# # TransactionListByVendor_DATE_MACRO=This Fiscal Year-to-date

# # Columns / breakdown (often "Total" or "Months")
# TransactionListByVendor_COLUMNS=Total
# # or: TransactionListByVendor_SUMMARIZE_COLUMN_BY=Months

# # Filters (optional)
# TransactionListByVendor_ACCOUNTING_METHOD=Accrual
# TransactionListByVendor_CURRENCY=USD
# TransactionListByVendor_VENDOR_IDS=100,101,102
# TransactionListByVendor_ACCOUNT_IDS=1,2,3
# TransactionListByVendor_CUSTOMER_IDS=10,11
# TransactionListByVendor_EMPLOYEE_IDS=30,31
# TransactionListByVendor_CLASS_IDS=40,41
# TransactionListByVendor_DEPARTMENT_IDS=50,51
# TransactionListByVendor_PAYMENT_METHOD_IDS=60,61
# TransactionListByVendor_NAME_IDS=70,71
# TransactionListByVendor_TRANSACTION_TYPE=Bill,BillPaymentCheck,Check,Expense,JournalEntry

# # Paging & insert behavior
# TransactionListByVendor_STARTPOSITION=1
# TransactionListByVendor_PAGESIZE=2000
# TransactionListByVendor_FORCE_TEXT_INSERT=false
# TransactionListByVendor_COERCE_TABLE_TO_TEXT=true
