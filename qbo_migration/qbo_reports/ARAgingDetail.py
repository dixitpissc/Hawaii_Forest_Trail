...
REPORT_TABLE  = os.getenv("ARAgingDetail_TABLE", "ARAgingDetail")
SAVE_TO_SQL   = os.getenv("ARAgingDetail_SAVE_TO_SQL", "true").lower() == "true"

# Report options (only sent when provided)
AS_OF_DATE    = os.getenv("ARAgingDetail_AS_OF_DATE")                # yyyy-mm-dd
DATE_MACRO    = os.getenv("ARAgingDetail_DATE_MACRO")                # e.g. "This Fiscal Year-to-date"
CUSTOMER_IDS  = os.getenv("ARAgingDetail_CUSTOMER_IDS")              # "123,456"
ACCOUNTING    = os.getenv("ARAgingDetail_ACCOUNTING_METHOD")         # "Cash"|"Accrual"
CURRENCY      = os.getenv("ARAgingDetail_CURRENCY")                  # "USD"|"CAD"|...
MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (if ignored by report, QBO safely discards)
START_POS     = int(os.getenv("ARAgingDetail_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("ARAgingDetail_PAGESIZE", "1000"))
...
REPORT_NAME = "AgedReceivableDetail"  # ← A/R Aging Detail
...
def _extract_columns_meta(report: dict):
    ...
    And a list of output columns (values + ids) for a lossless flatten:
    ...
    for i, c in enumerate(cols):
        ...
        out_columns.append(base)               # value
        out_columns.append(f"{base}__Id")      # id (often empty but keep for parity)
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching A/R Aging Detail from QBO")
    try:
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("ARAgingDetail_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ ARAgingDetail: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_ar_aging_detail():
    logger.info("🚀 Starting A/R Aging Detail report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (ARAgingDetail_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ ARAgingDetail report failed: {msg[:2000]}")
        raise
...
#Env knobs you can set (examples):
# ARAgingDetail_SAVE_TO_SQL=true
# ARAgingDetail_SAVE_EMPTY_SCHEMA=true
# ARAgingDetail_AS_OF_DATE=2025-08-15
# ARAgingDetail_DATE_MACRO=This Fiscal Year-to-date
# ARAgingDetail_ACCOUNTING_METHOD=Accrual
# ARAgingDetail_CURRENCY=USD
# ARAgingDetail_CUSTOMER_IDS=123,456
# ARAgingDetail_STARTPOSITION=1
# ARAgingDetail_PAGESIZE=1000
