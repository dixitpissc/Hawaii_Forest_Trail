...
REPORT_TABLE  = os.getenv("InventoryValuationDetail_TABLE", "InventoryValuationDetail")
SAVE_TO_SQL   = os.getenv("InventoryValuationDetail_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("InventoryValuationDetail_START_DATE")       # yyyy-mm-dd
END_DATE      = os.getenv("InventoryValuationDetail_END_DATE")         # yyyy-mm-dd
DATE_MACRO    = os.getenv("InventoryValuationDetail_DATE_MACRO")       # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("InventoryValuationDetail_ACCOUNTING_METHOD")# "Cash"|"Accrual"
CURRENCY      = os.getenv("InventoryValuationDetail_CURRENCY")         # "USD"|"CAD"|...

# Common entity filters supported by Reports API
ITEM_IDS      = os.getenv("InventoryValuationDetail_ITEM_IDS")         # "100,105,120"
CLASS_IDS     = os.getenv("InventoryValuationDetail_CLASS_IDS")        # "200,201"
DEPT_IDS      = os.getenv("InventoryValuationDetail_DEPARTMENT_IDS")   # "1,2" (aka 'location' in UI)

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; safe to include)
START_POS     = int(os.getenv("InventoryValuationDetail_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("InventoryValuationDetail_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("InventoryValuationDetail_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("InventoryValuationDetail_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
REPORT_NAME = "InventoryValuationDetail"
...
def _walk_rows_dynamic(rows_container: dict, col_meta: list, current_group_path: list, rows_out: list):
    ...
    for r in rows:
        ...
        if rtype == "Section":
            ...
            # Optional section summary (e.g., per-item totals)
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Inventory Valuation Detail from QBO")
    try:
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("InventoryValuationDetail_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ InventoryValuationDetail: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_inventory_valuation_detail():
    logger.info("🚀 Starting Inventory Valuation Detail report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (InventoryValuationDetail_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ InventoryValuationDetail report failed: {msg[:2000]}")
        raise
...
# InventoryValuationDetail_SAVE_TO_SQL=true
# InventoryValuationDetail_SAVE_EMPTY_SCHEMA=true

# # Date range or macro
# InventoryValuationDetail_START_DATE=2024-01-01
# InventoryValuationDetail_END_DATE=2025-09-02
# # or
# # InventoryValuationDetail_DATE_MACRO=This Fiscal Year-to-date

# # Filters
# InventoryValuationDetail_ITEM_IDS=100,105,120
# InventoryValuationDetail_CLASS_IDS=200,201
# InventoryValuationDetail_DEPARTMENT_IDS=1,2

# InventoryValuationDetail_ACCOUNTING_METHOD=Accrual
# InventoryValuationDetail_CURRENCY=USD

# # Paging & insert behavior
# InventoryValuationDetail_STARTPOSITION=1
# InventoryValuationDetail_PAGESIZE=2000
# InventoryValuationDetail_FORCE_TEXT_INSERT=false
# InventoryValuationDetail_COERCE_TABLE_TO_TEXT=true
