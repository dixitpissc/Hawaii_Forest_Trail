...
REPORT_TABLE  = os.getenv("CustomerIncome_TABLE", "CustomerIncome")
SAVE_TO_SQL   = os.getenv("CustomerIncome_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("CustomerIncome_START_DATE")              # yyyy-mm-dd
END_DATE      = os.getenv("CustomerIncome_END_DATE")                # yyyy-mm-dd
DATE_MACRO    = os.getenv("CustomerIncome_DATE_MACRO")              # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("CustomerIncome_ACCOUNTING_METHOD")       # "Cash"|"Accrual"
CURRENCY      = os.getenv("CustomerIncome_CURRENCY")                # "USD"|"CAD"|...
CUSTOMER_IDS  = os.getenv("CustomerIncome_CUSTOMER_IDS")            # "123,456"
MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; safe to include)
START_POS     = int(os.getenv("CustomerIncome_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("CustomerIncome_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("CustomerIncome_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("CustomerIncome_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Customer Income report from QBO")
    try:
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("CustomerIncome_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ CustomerIncome: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_customer_income():
    logger.info("🚀 Starting Customer Income report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (CustomerIncome_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ CustomerIncome report failed: {msg[:2000]}")
        raise



#### env example
# CustomerIncome_SAVE_TO_SQL=true
# CustomerIncome_SAVE_EMPTY_SCHEMA=true
# CustomerIncome_START_DATE=2024-01-01
# CustomerIncome_END_DATE=2025-09-02
# # or use a macro instead of explicit dates:
# CustomerIncome_DATE_MACRO=This Fiscal Year-to-date
# CustomerIncome_ACCOUNTING_METHOD=Accrual
# CustomerIncome_CURRENCY=USD
# CustomerIncome_CUSTOMER_IDS=123,456
# CustomerIncome_STARTPOSITION=1
# CustomerIncome_PAGESIZE=1000
# CustomerIncome_FORCE_TEXT_INSERT=false
# CustomerIncome_COERCE_TABLE_TO_TEXT=true
