...
# Project utilities (same helpers you’re already using)
from utils.token_refresher import auto_refresh_token_if_needed
from utils.log_timer import global_logger as logger, ProgressTimer
from storage.sqlserver import sql

getcontext().prec = 28  # safe precision for Decimal math
...
REPORT_TABLE  = os.getenv("TransactionListWithSplits_TABLE", "TransactionListWithSplits")
SAVE_TO_SQL   = os.getenv("TransactionListWithSplits_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("TransactionListWithSplits_START_DATE")               # yyyy-mm-dd
END_DATE      = os.getenv("TransactionListWithSplits_END_DATE")                 # yyyy-mm-dd
DATE_MACRO    = os.getenv("TransactionListWithSplits_DATE_MACRO")               # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("TransactionListWithSplits_ACCOUNTING_METHOD")        # "Cash"|"Accrual"
CURRENCY      = os.getenv("TransactionListWithSplits_CURRENCY")                 # "USD"|"CAD"|...
COLUMNS_BY    = os.getenv("TransactionListWithSplits_COLUMNS")                  # e.g., "Total" or "Months"
SUMMARIZE_BY  = os.getenv("TransactionListWithSplits_SUMMARIZE_COLUMN_BY")      # legacy alias, if you use it

# Filters commonly useful for this report
ACCOUNT_IDS   = os.getenv("TransactionListWithSplits_ACCOUNT_IDS")              # CSV of Account Ids
CUSTOMER_IDS  = os.getenv("TransactionListWithSplits_CUSTOMER_IDS")             # CSV
VENDOR_IDS    = os.getenv("TransactionListWithSplits_VENDOR_IDS")               # CSV
EMPLOYEE_IDS  = os.getenv("TransactionListWithSplits_EMPLOYEE_IDS")             # CSV
CLASS_IDS     = os.getenv("TransactionListWithSplits_CLASS_IDS")                # CSV
DEPT_IDS      = os.getenv("TransactionListWithSplits_DEPARTMENT_IDS")           # CSV
PAYMENTMETHOD_IDS = os.getenv("TransactionListWithSplits_PAYMENT_METHOD_IDS")   # CSV
NAME_IDS      = os.getenv("TransactionListWithSplits_NAME_IDS")                 # CSV (generic Name filter)
TXN_TYPE      = os.getenv("TransactionListWithSplits_TRANSACTION_TYPE")         # e.g., "Bill,JournalEntry,Expense" (CSV)

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; harmless if so)
START_POS     = int(os.getenv("TransactionListWithSplits_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("TransactionListWithSplits_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("TransactionListWithSplits_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("TransactionListWithSplits_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
REPORT_NAME = "TransactionListWithSplits"
...
def _build_params() -> dict:
    ...
    # Filters
    if ACCOUNT_IDS:        p["account"]        = ACCOUNT_IDS
    if CUSTOMER_IDS:       p["customer"]       = CUSTOMER_IDS
    if VENDOR_IDS:         p["vendor"]         = VENDOR_IDS
    if EMPLOYEE_IDS:       p["employee"]       = EMPLOYEE_IDS
    if CLASS_IDS:          p["class"]          = CLASS_IDS
    if DEPT_IDS:           p["department"]     = DEPT_IDS
    if PAYMENTMETHOD_IDS:  p["payment_method"] = PAYMENTMETHOD_IDS
    if NAME_IDS:           p["name"]           = NAME_IDS
    if TXN_TYPE:           p["transaction_type"] = TXN_TYPE  # comma-separated
...
def fetch_report_page(startposition: int = 1) -> dict:
    ...
    resp = requests.get(url, headers=_build_headers(access_token), params=params, timeout=120)
...
def _walk_rows_dynamic(rows_container: dict, col_meta: list, current_group_path: list, rows_out: list):
    ...
    for r in rows:
        ...
        if rtype == "Section":
            ...
            # Optional per-section summary (e.g., per account/name)
...
def _append_top_level_summary(report: dict, col_meta: list, rows_out: list):
    """Capture Rows.Summary (grand totals) as RowType='ReportSummary'."""
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Transaction List with Splits from QBO")
    try:
    ...
    # Optional: add a paging loop if your realm paginates this report
    return df
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("TransactionListWithSplits_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ TransactionListWithSplits: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_transaction_list_with_splits():
    logger.info("🚀 Starting Transaction List with Splits report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (TransactionListWithSplits_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ TransactionListWithSplits report failed: {msg[:2000]}")
        raise
...
# Env Credential
# TransactionListWithSplits_SAVE_TO_SQL=true
# TransactionListWithSplits_SAVE_EMPTY_SCHEMA=true

# # Date range or macro (this report is range-based)
# TransactionListWithSplits_START_DATE=2024-01-01
# TransactionListWithSplits_END_DATE=2024-12-31
# # or
# # TransactionListWithSplits_DATE_MACRO=This Fiscal Year-to-date

# # Columns / breakdown (often "Total" or "Months")
# TransactionListWithSplits_COLUMNS=Total
# # or: TransactionListWithSplits_SUMMARIZE_COLUMN_BY=Months

# # Filters (optional)
# TransactionListWithSplits_ACCOUNTING_METHOD=Accrual
# TransactionListWithSplits_CURRENCY=USD
# TransactionListWithSplits_ACCOUNT_IDS=1,2,3
# TransactionListWithSplits_CUSTOMER_IDS=10,11
# TransactionListWithSplits_VENDOR_IDS=20,21
# TransactionListWithSplits_EMPLOYEE_IDS=30,31
# TransactionListWithSplits_CLASS_IDS=40,41
# TransactionListWithSplits_DEPARTMENT_IDS=50,51
# TransactionListWithSplits_PAYMENT_METHOD_IDS=60,61
# TransactionListWithSplits_NAME_IDS=70,71
# TransactionListWithSplits_TRANSACTION_TYPE=Bill,JournalEntry,Expense,Check,VendorCredit

# # Paging & insert behavior
# TransactionListWithSplits_STARTPOSITION=1
# TransactionListWithSplits_PAGESIZE=2000
# TransactionListWithSplits_FORCE_TEXT_INSERT=false
# TransactionListWithSplits_COERCE_TABLE_TO_TEXT=true
