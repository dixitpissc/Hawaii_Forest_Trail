...
REPORT_TABLE  = os.getenv("InventoryValuationSummary_TABLE", "InventoryValuationSummary")
SAVE_TO_SQL   = os.getenv("InventoryValuationSummary_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("InventoryValuationSummary_START_DATE")        # yyyy-mm-dd
END_DATE      = os.getenv("InventoryValuationSummary_END_DATE")          # yyyy-mm-dd
DATE_MACRO    = os.getenv("InventoryValuationSummary_DATE_MACRO")        # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("InventoryValuationSummary_ACCOUNTING_METHOD") # "Cash"|"Accrual"
CURRENCY      = os.getenv("InventoryValuationSummary_CURRENCY")          # "USD"|"CAD"|...

# Common entity filters supported by Reports API
ITEM_IDS      = os.getenv("InventoryValuationSummary_ITEM_IDS")          # "100,105,120"
CLASS_IDS     = os.getenv("InventoryValuationSummary_CLASS_IDS")         # "200,201"
DEPT_IDS      = os.getenv("InventoryValuationSummary_DEPARTMENT_IDS")    # "1,2" (aka 'location' in UI)

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")  # keep >= 75

# Pagination (many reports ignore; safe to include)
START_POS     = int(os.getenv("InventoryValuationSummary_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("InventoryValuationSummary_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("InventoryValuationSummary_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("InventoryValuationSummary_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
REPORT_NAME = "InventoryValuationSummary"
...
def _extract_columns_meta(report: dict):
    ...
    Return meta + names like:
      [{"idx":0,"title":"Item","colkey":"item_name","safe":"Item__item_name"}, ...]
    and output columns (both value + Id):
      ["Item__item_name", "Item__item_name__Id", ...]
    """
...
def _append_top_level_summary(report: dict, col_meta: list, rows_out: list):
    """
    Some summary reports include a top-level Rows.Summary (grand totals).
    Capture it as a synthetic row with RowType='ReportSummary'.
    """
...
def flatten_report(report_obj: dict) -> pd.DataFrame:
    ...
    # capture a possible grand total
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Inventory Valuation Summary from QBO")
    try:
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("InventoryValuationSummary_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ InventoryValuationSummary: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_inventory_valuation_summary():
    logger.info("🚀 Starting Inventory Valuation Summary report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (InventoryValuationSummary_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ InventoryValuationSummary report failed: {msg[:2000]}")
        raise
...
# InventoryValuationSummary_SAVE_TO_SQL=true
# InventoryValuationSummary_SAVE_EMPTY_SCHEMA=true

# # Date range or macro
# InventoryValuationSummary_START_DATE=2010-01-01
# InventoryValuationSummary_END_DATE=2025-09-02
# # or
# # InventoryValuationSummary_DATE_MACRO=This Fiscal Year-to-date

# # Filters
# InventoryValuationSummary_ITEM_IDS=100,105,120
# InventoryValuationSummary_CLASS_IDS=200,201
# InventoryValuationSummary_DEPARTMENT_IDS=1,2

# InventoryValuationSummary_ACCOUNTING_METHOD=Accrual
# InventoryValuationSummary_CURRENCY=USD

# # Paging & insert behavior
# InventoryValuationSummary_STARTPOSITION=1
# InventoryValuationSummary_PAGESIZE=2000
# InventoryValuationSummary_FORCE_TEXT_INSERT=false
# InventoryValuationSummary_COERCE_TABLE_TO_TEXT=true
