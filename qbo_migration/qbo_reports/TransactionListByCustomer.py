...
REPORT_TABLE  = os.getenv("TransactionListByCustomer_TABLE", "TransactionListByCustomer")
SAVE_TO_SQL   = os.getenv("TransactionListByCustomer_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("TransactionListByCustomer_START_DATE")               # yyyy-mm-dd
END_DATE      = os.getenv("TransactionListByCustomer_END_DATE")                 # yyyy-mm-dd
DATE_MACRO    = os.getenv("TransactionListByCustomer_DATE_MACRO")               # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("TransactionListByCustomer_ACCOUNTING_METHOD")        # "Cash"|"Accrual"
CURRENCY      = os.getenv("TransactionListByCustomer_CURRENCY")                 # "USD"|"CAD"|...
COLUMNS_BY    = os.getenv("TransactionListByCustomer_COLUMNS")                  # e.g., "Total" or "Months"
SUMMARIZE_BY  = os.getenv("TransactionListByCustomer_SUMMARIZE_COLUMN_BY")      # legacy alias, if you use it

# Filters (QBO ignores unrecognized gracefully)
CUSTOMER_IDS  = os.getenv("TransactionListByCustomer_CUSTOMER_IDS")             # CSV
ACCOUNT_IDS   = os.getenv("TransactionListByCustomer_ACCOUNT_IDS")              # CSV
VENDOR_IDS    = os.getenv("TransactionListByCustomer_VENDOR_IDS")               # CSV
EMPLOYEE_IDS  = os.getenv("TransactionListByCustomer_EMPLOYEE_IDS")             # CSV
CLASS_IDS     = os.getenv("TransactionListByCustomer_CLASS_IDS")                # CSV
DEPT_IDS      = os.getenv("TransactionListByCustomer_DEPARTMENT_IDS")           # CSV
PAYMENTMETHOD_IDS = os.getenv("TransactionListByCustomer_PAYMENT_METHOD_IDS")   # CSV
NAME_IDS      = os.getenv("TransactionListByCustomer_NAME_IDS")                 # CSV (generic Name filter)
TXN_TYPE      = os.getenv("TransactionListByCustomer_TRANSACTION_TYPE")         # e.g., "Invoice,Payment,SalesReceipt" (CSV)

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; harmless if so)
START_POS     = int(os.getenv("TransactionListByCustomer_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("TransactionListByCustomer_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("TransactionListByCustomer_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("TransactionListByCustomer_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
REPORT_NAME = "TransactionListByCustomer"
...
def _walk_rows_dynamic(rows_container: dict, col_meta: list, current_group_path: list, rows_out: list):
    ...
    for r in rows:
        ...
        if rtype == "Section":
            ...
            # Optional per-section summary (e.g., per customer)
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Transaction List by Customer from QBO")
    try:
    ...
    # Optional: add paging loop here if your realm paginates this report
    return df
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("TransactionListByCustomer_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ TransactionListByCustomer: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_transaction_list_by_customer():
    logger.info("🚀 Starting Transaction List by Customer report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (TransactionListByCustomer_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ TransactionListByCustomer report failed: {msg[:2000]}")
        raise
...
# TransactionListByCustomer_SAVE_TO_SQL=true
# TransactionListByCustomer_SAVE_EMPTY_SCHEMA=true

# # Date range or macro
# TransactionListByCustomer_START_DATE=2024-01-01
# TransactionListByCustomer_END_DATE=2024-12-31
# # or
# # TransactionListByCustomer_DATE_MACRO=This Fiscal Year-to-date

# # Columns / breakdown
# TransactionListByCustomer_COLUMNS=Total
# # or: TransactionListByCustomer_SUMMARIZE_COLUMN_BY=Months

# # Filters (optional)
# TransactionListByCustomer_ACCOUNTING_METHOD=Accrual
# TransactionListByCustomer_CURRENCY=USD
# TransactionListByCustomer_CUSTOMER_IDS=100,101,102
# TransactionListByCustomer_ACCOUNT_IDS=1,2,3
# TransactionListByCustomer_VENDOR_IDS=10,11
# TransactionListByCustomer_EMPLOYEE_IDS=30,31
# TransactionListByCustomer_CLASS_IDS=40,41
# TransactionListByCustomer_DEPARTMENT_IDS=50,51
# TransactionListByCustomer_PAYMENT_METHOD_IDS=60,61
# TransactionListByCustomer_NAME_IDS=70,71
# TransactionListByCustomer_TRANSACTION_TYPE=Invoice,Payment,SalesReceipt,JournalEntry

# # Paging & insert behavior
# TransactionListByCustomer_STARTPOSITION=1
# TransactionListByCustomer_PAGESIZE=2000
# TransactionListByCustomer_FORCE_TEXT_INSERT=false
# TransactionListByCustomer_COERCE_TABLE_TO_TEXT=true
