...
REPORT_TABLE  = os.getenv("VendorBalance_TABLE", "VendorBalance")
SAVE_TO_SQL   = os.getenv("VendorBalance_SAVE_TO_SQL", "true").lower() == "true"

# Report options (only sent when provided)
START_DATE    = os.getenv("VendorBalance_START_DATE")               # yyyy-mm-dd
END_DATE      = os.getenv("VendorBalance_END_DATE")                 # yyyy-mm-dd
DATE_MACRO    = os.getenv("VendorBalance_DATE_MACRO")               # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("VendorBalance_ACCOUNTING_METHOD")        # "Cash"|"Accrual"
CURRENCY      = os.getenv("VendorBalance_CURRENCY")                 # "USD"|"CAD"|...
COLUMNS_BY    = os.getenv("VendorBalance_COLUMNS")                  # e.g., "Total" or "Months"
SUMMARIZE_BY  = os.getenv("VendorBalance_SUMMARIZE_COLUMN_BY")      # legacy alias if you use it

# Filters (QBO ignores unrecognized gracefully)
VENDOR_IDS    = os.getenv("VendorBalance_VENDOR_IDS")               # CSV of Vendor.Id
ACCOUNT_IDS   = os.getenv("VendorBalance_ACCOUNT_IDS")              # CSV
CLASS_IDS     = os.getenv("VendorBalance_CLASS_IDS")                # CSV
DEPT_IDS      = os.getenv("VendorBalance_DEPARTMENT_IDS")           # CSV
NAME_IDS      = os.getenv("VendorBalance_NAME_IDS")                 # CSV (generic Name filter)

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (most pulls are single page; harmless if ignored)
START_POS     = int(os.getenv("VendorBalance_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("VendorBalance_PAGESIZE", "2000"))

# Insert behavior
FORCE_TEXT    = os.getenv("VendorBalance_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("VendorBalance_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
def _build_params() -> dict:
    ...
    # Prefer explicit range; else macro. (If only END_DATE is set, QBO often treats as "as of".)
...
def _extract_columns_meta(report: dict):
    ...
    Return meta + names like:
      [{"idx":0,"title":"Vendor","colkey":"name","safe":"Vendor__name"}, ...]
    and output columns (value + Id):
      ["Vendor__name", "Vendor__name__Id", ...]
    """
...
def _walk_rows_dynamic(rows_container: dict, col_meta: list, current_group_path: list, rows_out: list):
    ...
    for r in rows:
        ...
        if rtype == "Section":
            ...
            # Optional per-section summary (e.g., per vendor totals)
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Vendor Balance from QBO")
    try:
    ...
    # Optional: add paging loop if your realm paginates this report
    return df
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("VendorBalance_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ VendorBalance: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_vendor_balance():
    logger.info("🚀 Starting Vendor Balance report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (VendorBalance_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ VendorBalance report failed: {msg[:2000]}")
        raise
...
# VendorBalance_SAVE_TO_SQL=true
# VendorBalance_SAVE_EMPTY_SCHEMA=true

# # Date range or macro (set one of these approaches)
# VendorBalance_START_DATE=2025-01-01
# VendorBalance_END_DATE=2025-09-02
# # or
# # VendorBalance_DATE_MACRO=This Fiscal Year-to-date

# # Columns / breakdown
# VendorBalance_COLUMNS=Total
# # or: VendorBalance_SUMMARIZE_COLUMN_BY=Months

# # Filters (optional)
# VendorBalance_ACCOUNTING_METHOD=Accrual
# VendorBalance_CURRENCY=USD
# VendorBalance_VENDOR_IDS=100,101,102
# VendorBalance_ACCOUNT_IDS=1,2,3
# VendorBalance_CLASS_IDS=10,11
# VendorBalance_DEPARTMENT_IDS=20,21
# VendorBalance_NAME_IDS=300,301

# # Paging & insert behavior
# VendorBalance_STARTPOSITION=1
# VendorBalance_PAGESIZE=2000
# VendorBalance_FORCE_TEXT_INSERT=false
# VendorBalance_COERCE_TABLE_TO_TEXT=true
