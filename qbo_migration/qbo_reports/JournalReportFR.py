...
REPORT_TABLE  = os.getenv("JournalReportFR_TABLE", "JournalReportFR")
SAVE_TO_SQL   = os.getenv("JournalReportFR_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("JournalReportFR_START_DATE")               # yyyy-mm-dd
END_DATE      = os.getenv("JournalReportFR_END_DATE")                 # yyyy-mm-dd
DATE_MACRO    = os.getenv("JournalReportFR_DATE_MACRO")               # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("JournalReportFR_ACCOUNTING_METHOD")        # "Cash"|"Accrual"
CURRENCY      = os.getenv("JournalReportFR_CURRENCY")                 # "EUR"|"USD"|...
ACCOUNT_IDS   = os.getenv("JournalReportFR_ACCOUNT_IDS")              # CSV of QBO Account Ids
TXN_TYPES     = os.getenv("JournalReportFR_TRANSACTION_TYPES")        # CSV: JournalEntry,Invoice,Bill,...
POSTING_TYPE  = os.getenv("JournalReportFR_POSTING_TYPE")             # "Posting"|"NonPosting"
COLUMNS_BY    = os.getenv("JournalReportFR_COLUMNS")                  # optional explicit columns
NAME_IDS      = os.getenv("JournalReportFR_NAME_IDS")                 # CSV of Name (entity) Ids
CLASS_IDS     = os.getenv("JournalReportFR_CLASS_IDS")
DEPT_IDS      = os.getenv("JournalReportFR_DEPARTMENT_IDS")
JOURNAL_CODES = os.getenv("JournalReportFR_JOURNAL_CODES")            # locale-specific: CSV of journal codes

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; safe to include)
START_POS     = int(os.getenv("JournalReportFR_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("JournalReportFR_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("JournalReportFR_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("JournalReportFR_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
def _build_params() -> dict:
    ...
    if JOURNAL_CODES: p["journal_code"]      = JOURNAL_CODES  # FR-specific (if your edition supports it)
...
def _walk_rows_dynamic(rows_container: dict, col_meta: list, current_group_path: list, rows_out: list):
    ...
    for r in rows:
        ...
        if rtype == "Section":
            ...
            # Optional section summary (e.g., per-account/per-journal subtotals)
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Journal Report FR from QBO")
    try:
    ...
    # Optional paging loop pattern is identical to your other modules
    return df
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("JournalReportFR_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ JournalReportFR: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_journal_report_fr():
    logger.info("🚀 Starting Journal Report FR pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (JournalReportFR_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ JournalReportFR failed: {msg[:2000]}")
        raise
