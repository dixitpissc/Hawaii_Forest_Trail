...
REPORT_TABLE  = os.getenv("CustomerBalanceDetail_TABLE", "CustomerBalanceDetail")
SAVE_TO_SQL   = os.getenv("CustomerBalanceDetail_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("CustomerBalanceDetail_START_DATE")       # yyyy-mm-dd
END_DATE      = os.getenv("CustomerBalanceDetail_END_DATE")         # yyyy-mm-dd
DATE_MACRO    = os.getenv("CustomerBalanceDetail_DATE_MACRO")       # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("CustomerBalanceDetail_ACCOUNTING_METHOD")# "Cash"|"Accrual"
CURRENCY      = os.getenv("CustomerBalanceDetail_CURRENCY")         # "USD"|"CAD"|...
CUSTOMER_IDS  = os.getenv("CustomerBalanceDetail_CUSTOMER_IDS")     # "123,456"
MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; safe to include)
START_POS     = int(os.getenv("CustomerBalanceDetail_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("CustomerBalanceDetail_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("CustomerBalanceDetail_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("CustomerBalanceDetail_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Customer Balance Detail from QBO")
    try:
    ...
    # Optional: implement true paging loop if your tenant returns > PAGE_SIZE rows
    # frames = [df]
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("CustomerBalanceDetail_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ CustomerBalanceDetail: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_customer_balance_detail():
    logger.info("🚀 Starting Customer Balance Detail report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (CustomerBalanceDetail_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ CustomerBalanceDetail report failed: {msg[:2000]}")
        raise
