...
REPORT_TABLE  = os.getenv("SalesByProduct_TABLE", "SalesByProduct")
SAVE_TO_SQL   = os.getenv("SalesByProduct_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("SalesByProduct_START_DATE")               # yyyy-mm-dd
END_DATE      = os.getenv("SalesByProduct_END_DATE")                 # yyyy-mm-dd
DATE_MACRO    = os.getenv("SalesByProduct_DATE_MACRO")               # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("SalesByProduct_ACCOUNTING_METHOD")        # "Cash"|"Accrual"
CURRENCY      = os.getenv("SalesByProduct_CURRENCY")                 # "USD"|"CAD"|...

# Column breakdowns (Months, Total, etc.)
COLUMNS_BY    = os.getenv("SalesByProduct_COLUMNS")                  # e.g., "Months" or "Total"
SUMMARIZE_BY  = os.getenv("SalesByProduct_SUMMARIZE_COLUMN_BY")      # legacy alias if you use it

# Filters (QBO ignores unknowns gracefully)
ITEM_IDS      = os.getenv("SalesByProduct_ITEM_IDS")                 # CSV
CUSTOMER_IDS  = os.getenv("SalesByProduct_CUSTOMER_IDS")             # CSV
CLASS_IDS     = os.getenv("SalesByProduct_CLASS_IDS")                # CSV
DEPT_IDS      = os.getenv("SalesByProduct_DEPARTMENT_IDS")           # CSV
NAME_IDS      = os.getenv("SalesByProduct_NAME_IDS")                 # CSV

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; safe to include)
START_POS     = int(os.getenv("SalesByProduct_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("SalesByProduct_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("SalesByProduct_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("SalesByProduct_COERCE_TABLE_TO_TEXT", "true").lower() == "true"

# Allow overriding the endpoint name if needed (docs vs. actual)
REPORT_NAME_OVERRIDE = os.getenv("SalesByProduct_ENDPOINT")          # e.g., "SalesByProduct"
...
# Default to ItemSales; optionally override to SalesByProduct if you prefer.
REPORT_NAME = REPORT_NAME_OVERRIDE or "ItemSales"
...
def _extract_columns_meta(report: dict):
    ...
    Return meta + names like:
      [{"idx":0,"title":"Product/Service","colkey":"item_name","safe":"Product_Service__item_name"}, ...]
    and output columns (value + Id):
      ["Product_Service__item_name", "Product_Service__item_name__Id", ...]
    """
...
def _walk_rows_dynamic(rows_container: dict, col_meta: list, current_group_path: list, rows_out: list):
    ...
    for r in rows:
        ...
        if rtype == "Section":
            ...
            # Optional section summary (per-item totals)
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Sales by Product/Service from QBO")
    try:
    ...
    # Optional: add paging loop if your tenant paginates this report
    return df
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("SalesByProduct_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ SalesByProduct: Nothing to save (empty DataFrame).")
        return
    ...
    try:
        ...
        if COERCE_TABLE:
            try:
                ...
                logger.info(f"🔧 Coerced [{schema}].[{table}] to NVARCHAR(255); retrying insert…")
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_sales_by_product():
    logger.info("🚀 Starting Sales by Product/Service report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (SalesByProduct_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ SalesByProduct report failed: {msg[:2000]}")
        raise
...
# SalesByProduct_SAVE_TO_SQL=true
# SalesByProduct_SAVE_EMPTY_SCHEMA=true

# # Date range or macro
# SalesByProduct_START_DATE=2024-01-01
# SalesByProduct_END_DATE=2024-12-31
# # or
# # SalesByProduct_DATE_MACRO=This Fiscal Year-to-date

# # Columns / breakdown (e.g., Months or Total)
# SalesByProduct_COLUMNS=Months
# # or: SalesByProduct_SUMMARIZE_COLUMN_BY=Months

# # Filters (optional)
# SalesByProduct_ACCOUNTING_METHOD=Accrual
# SalesByProduct_CURRENCY=USD
# SalesByProduct_ITEM_IDS=200,201,202
# SalesByProduct_CUSTOMER_IDS=123,456
# SalesByProduct_CLASS_IDS=10,11
# SalesByProduct_DEPARTMENT_IDS=1,2
# SalesByProduct_NAME_IDS=888,889

# # Paging & insert behavior
# SalesByProduct_STARTPOSITION=1
# SalesByProduct_PAGESIZE=2000
# SalesByProduct_FORCE_TEXT_INSERT=false
# SalesByProduct_COERCE_TABLE_TO_TEXT=true
