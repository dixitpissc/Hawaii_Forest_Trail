...
REPORT_TABLE  = os.getenv("TrialBalance_TABLE", "TrialBalance")
SAVE_TO_SQL   = os.getenv("TrialBalance_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("TrialBalance_START_DATE")               # yyyy-mm-dd
END_DATE      = os.getenv("TrialBalance_END_DATE")                 # yyyy-mm-dd
DATE_MACRO    = os.getenv("TrialBalance_DATE_MACRO")               # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("TrialBalance_ACCOUNTING_METHOD")        # "Accrual" (TB is accrual-style; QBO may ignore)
CURRENCY      = os.getenv("TrialBalance_CURRENCY")                 # "USD"|"CAD"|...
COLUMNS_BY    = os.getenv("TrialBalance_COLUMNS")                  # e.g., "Total" (default) or "Months"
SUMMARIZE_BY  = os.getenv("TrialBalance_SUMMARIZE_COLUMN_BY")      # legacy alias if you use it

# Filters often accepted by Reports API (ignored if not applicable)
ACCOUNT_IDS   = os.getenv("TrialBalance_ACCOUNT_IDS")              # CSV
CLASS_IDS     = os.getenv("TrialBalance_CLASS_IDS")                # CSV
DEPT_IDS      = os.getenv("TrialBalance_DEPARTMENT_IDS")           # CSV
NAME_IDS      = os.getenv("TrialBalance_NAME_IDS")                 # CSV

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (most TB pulls are single page; harmless if ignored)
START_POS     = int(os.getenv("TrialBalance_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("TrialBalance_PAGESIZE", "2000"))

# Insert behavior
FORCE_TEXT    = os.getenv("TrialBalance_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("TrialBalance_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
def _build_params() -> dict:
    ...
    # Prefer explicit range; else macro. (TB supports start/end period;
    # if you only pass END_DATE, QBO treats it as “as of”.)
    ...
    # Filters (ignored if not applicable)
...
def _walk_rows_dynamic(rows_container: dict, col_meta: list, current_group_path: list, rows_out: list):
    ...
    for r in rows:
        ...
        if rtype == "Section":
            ...
            # Optional per-section summary (e.g., totals by account type)
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Trial Balance from QBO")
    try:
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("TrialBalance_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ TrialBalance: Nothing to save (empty DataFrame).")
        return
...
def run_trial_balance():
    logger.info("🚀 Starting Trial Balance report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (TrialBalance_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ TrialBalance report failed: {msg[:2000]}")
        raise
