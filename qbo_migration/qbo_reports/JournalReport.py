...
REPORT_TABLE  = os.getenv("JournalReport_TABLE", "JournalReport")
SAVE_TO_SQL   = os.getenv("JournalReport_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("JournalReport_START_DATE")               # yyyy-mm-dd
END_DATE      = os.getenv("JournalReport_END_DATE")                 # yyyy-mm-dd
DATE_MACRO    = os.getenv("JournalReport_DATE_MACRO")               # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("JournalReport_ACCOUNTING_METHOD")        # "Cash"|"Accrual"
CURRENCY      = os.getenv("JournalReport_CURRENCY")                 # "USD"|"CAD"|...
ACCOUNT_IDS   = os.getenv("JournalReport_ACCOUNT_IDS")              # "10,12,21" (QBO Account Ids)
TXN_TYPES     = os.getenv("JournalReport_TRANSACTION_TYPES")        # CSV: JournalEntry,Invoice,Bill,...
POSTING_TYPE  = os.getenv("JournalReport_POSTING_TYPE")             # "Posting"|"NonPosting"
COLUMNS_BY    = os.getenv("JournalReport_COLUMNS")                  # request explicit columns (optional)
NAME_IDS      = os.getenv("JournalReport_NAME_IDS")                 # optional CSV of Entity (Name) IDs
CLASS_IDS     = os.getenv("JournalReport_CLASS_IDS")
DEPT_IDS      = os.getenv("JournalReport_DEPARTMENT_IDS")

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; safe to include)
START_POS     = int(os.getenv("JournalReport_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("JournalReport_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("JournalReport_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("JournalReport_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
def _build_params() -> dict:
    ...
    # Prefer explicit range when provided; else date macro
    ...
    if POSTING_TYPE: p["posting_type"]      = POSTING_TYPE
    if COLUMNS_BY:   p["columns"]           = COLUMNS_BY

    # Common dimension filters (QBO ignores unknowns)
...
def _extract_columns_meta(report: dict):
    ...
    and output columns (both value + Id):
...
def _walk_rows_dynamic(rows_container: dict, col_meta: list, current_group_path: list, rows_out: list):
    ...
    for r in rows:
        ...
        if rtype == "Section":
            ...
            # Optional section summary (e.g., per-txn or per-account subtotals)
...
def _append_top_level_summary(report: dict, col_meta: list, rows_out: list):
    """Capture Rows.Summary (grand totals) as RowType='ReportSummary' when present."""
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Journal Report from QBO")
    try:
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("JournalReport_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ JournalReport: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_journal_report():
    logger.info("🚀 Starting Journal Report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (JournalReport_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ JournalReport failed: {msg[:2000]}")
        raise
...
# JournalReport_SAVE_TO_SQL=true
# JournalReport_SAVE_EMPTY_SCHEMA=true

# # Date range or macro
# JournalReport_START_DATE=2010-01-01
# JournalReport_END_DATE=2025-09-02
# # or
# # JournalReport_DATE_MACRO=This Fiscal Year-to-date

# # Filters (optional)
# JournalReport_ACCOUNTING_METHOD=Accrual
# JournalReport_CURRENCY=USD
# JournalReport_ACCOUNT_IDS=10,11,21
# JournalReport_TRANSACTION_TYPES=JournalEntry,Invoice,Bill,Payment
# JournalReport_POSTING_TYPE=Posting
# JournalReport_COLUMNS=Date,Txn_Type,Doc_Num,Account,Split,Debit,Credit,Amount
# JournalReport_NAME_IDS=123,456
# JournalReport_CLASS_IDS=200,201
# JournalReport_DEPARTMENT_IDS=1,2

# # Paging & insert behavior
# JournalReport_STARTPOSITION=1
# JournalReport_PAGESIZE=2000
# JournalReport_FORCE_TEXT_INSERT=false
# JournalReport_COERCE_TABLE_TO_TEXT=true
