...
REPORT_TABLE  = os.getenv("TransactionList_TABLE", "TransactionList")
SAVE_TO_SQL   = os.getenv("TransactionList_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("TransactionList_START_DATE")               # yyyy-mm-dd
END_DATE      = os.getenv("TransactionList_END_DATE")                 # yyyy-mm-dd
DATE_MACRO    = os.getenv("TransactionList_DATE_MACRO")               # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("TransactionList_ACCOUNTING_METHOD")        # "Cash"|"Accrual"
CURRENCY      = os.getenv("TransactionList_CURRENCY")                 # "USD"|"CAD"|...
COLUMNS_BY    = os.getenv("TransactionList_COLUMNS")                  # e.g., "Total" or "Months"
SUMMARIZE_BY  = os.getenv("TransactionList_SUMMARIZE_COLUMN_BY")      # legacy alias, if you use it

# Common filters (QBO ignores unrecognized ones gracefully)
ACCOUNT_IDS   = os.getenv("TransactionList_ACCOUNT_IDS")              # CSV of Account Ids
CUSTOMER_IDS  = os.getenv("TransactionList_CUSTOMER_IDS")             # CSV
VENDOR_IDS    = os.getenv("TransactionList_VENDOR_IDS")               # CSV
EMPLOYEE_IDS  = os.getenv("TransactionList_EMPLOYEE_IDS")             # CSV
CLASS_IDS     = os.getenv("TransactionList_CLASS_IDS")                # CSV
DEPT_IDS      = os.getenv("TransactionList_DEPARTMENT_IDS")           # CSV
PAYMENTMETHOD_IDS = os.getenv("TransactionList_PAYMENT_METHOD_IDS")   # CSV
NAME_IDS      = os.getenv("TransactionList_NAME_IDS")                 # CSV (generic Name filter)
TXN_TYPE      = os.getenv("TransactionList_TRANSACTION_TYPE")         # e.g., "Invoice,Bill,Payment" (comma sep)

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; harmless if so)
START_POS     = int(os.getenv("TransactionList_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("TransactionList_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("TransactionList_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("TransactionList_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
def _build_params() -> dict:
    ...
    # Filters (report accepts many; unsupported are ignored)
...
def _walk_rows_dynamic(rows_container: dict, col_meta: list, current_group_path: list, rows_out: list):
    ...
    for r in rows:
        ...
        if rtype == "Section":
            ...
            # Optional per-section summary (e.g., per-name or per-account)
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Transaction List from QBO")
    try:
    ...
    # Optional paging loop if your realm paginates this report
    return df
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("TransactionList_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ TransactionList: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_transaction_list():
    logger.info("🚀 Starting Transaction List report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (TransactionList_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ TransactionList report failed: {msg[:2000]}")
        raise
...
#Env credential
# TransactionList_SAVE_TO_SQL=true
# TransactionList_SAVE_EMPTY_SCHEMA=true

# # Date range or macro (Transaction List is range-based)
# TransactionList_START_DATE=2024-01-01
# TransactionList_END_DATE=2024-12-31
# # or
# # TransactionList_DATE_MACRO=This Fiscal Year-to-date

# # Columns / breakdown (often "Total" or "Months")
# TransactionList_COLUMNS=Total
# # or: TransactionList_SUMMARIZE_COLUMN_BY=Months

# # Filters (optional)
# TransactionList_ACCOUNTING_METHOD=Accrual
# TransactionList_CURRENCY=USD
# TransactionList_ACCOUNT_IDS=1,2,3
# TransactionList_CUSTOMER_IDS=10,11
# TransactionList_VENDOR_IDS=20,21
# TransactionList_EMPLOYEE_IDS=30,31
# TransactionList_CLASS_IDS=40,41
# TransactionList_DEPARTMENT_IDS=50,51
# TransactionList_PAYMENT_METHOD_IDS=60,61
# TransactionList_NAME_IDS=70,71
# TransactionList_TRANSACTION_TYPE=Invoice,Bill,Payment,JournalEntry

# # Paging & insert behavior
# TransactionList_STARTPOSITION=1
# TransactionList_PAGESIZE=2000
# TransactionList_FORCE_TEXT_INSERT=false
# TransactionList_COERCE_TABLE_TO_TEXT=true
