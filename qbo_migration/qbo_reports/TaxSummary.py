...
REPORT_TABLE  = os.getenv("TaxSummary_TABLE", "TaxSummary")
SAVE_TO_SQL   = os.getenv("TaxSummary_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("TaxSummary_START_DATE")               # yyyy-mm-dd
END_DATE      = os.getenv("TaxSummary_END_DATE")                 # yyyy-mm-dd
DATE_MACRO    = os.getenv("TaxSummary_DATE_MACRO")               # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("TaxSummary_ACCOUNTING_METHOD")        # "Cash"|"Accrual"
CURRENCY      = os.getenv("TaxSummary_CURRENCY")                 # "USD"|"CAD"|...
COLUMNS_BY    = os.getenv("TaxSummary_COLUMNS")                  # e.g., "Months" or "Total"
SUMMARIZE_BY  = os.getenv("TaxSummary_SUMMARIZE_COLUMN_BY")      # legacy alias, if you use it

# Common filters used by tax reports (unrecognized are ignored by QBO safely)
TAX_AGENCY_IDS = os.getenv("TaxSummary_TAX_AGENCY_IDS")          # CSV of TaxAgency Ids
TAX_CODE_IDS   = os.getenv("TaxSummary_TAX_CODE_IDS")            # CSV of TaxCode Ids
TAX_RATE_IDS   = os.getenv("TaxSummary_TAX_RATE_IDS")            # CSV of TaxRate Ids
DEPT_IDS       = os.getenv("TaxSummary_DEPARTMENT_IDS")          # CSV
CLASS_IDS      = os.getenv("TaxSummary_CLASS_IDS")               # CSV
NAME_IDS       = os.getenv("TaxSummary_NAME_IDS")                # CSV (generic Name filter)

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")

# Pagination (many reports ignore; harmless if so)
START_POS     = int(os.getenv("TaxSummary_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("TaxSummary_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("TaxSummary_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("TaxSummary_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
REPORT_NAME = "TaxSummary"  # QBO Reports API endpoint name
...
def _build_params() -> dict:
    ...
    # Likely filters supported by tax reports (ignored if not applicable)
    if TAX_AGENCY_IDS: p["tax_agency"] = TAX_AGENCY_IDS
    if TAX_CODE_IDS:   p["tax_code"]   = TAX_CODE_IDS
    if TAX_RATE_IDS:   p["tax_rate"]   = TAX_RATE_IDS
...
def _extract_columns_meta(report: dict):
    ...
    Return meta + names like:
      [{"idx":0,"title":"Tax Code","colkey":"tax_code","safe":"Tax_Code__tax_code"}, ...]
    and output columns (value + Id):
      ["Tax_Code__tax_code", "Tax_Code__tax_code__Id", ...]
    """
...
def _walk_rows_dynamic(rows_container: dict, col_meta: list, current_group_path: list, rows_out: list):
    ...
    for r in rows:
        ...
        if rtype == "Section":
            ...
            # Optional per-section summary (e.g., per tax code)
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Tax Summary from QBO")
    try:
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("TaxSummary_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ TaxSummary: Nothing to save (empty DataFrame).")
        return
...
# ──────────────────────────────────────────────────────────────
def run_tax_summary():
    logger.info("🚀 Starting Tax Summary report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (TaxSummary_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ TaxSummary report failed: {msg[:2000]}")
        raise
...
# TaxSummary_SAVE_TO_SQL=true
# TaxSummary_SAVE_EMPTY_SCHEMA=true

# # Date range or macro (use one style)
# TaxSummary_START_DATE=2024-01-01
# TaxSummary_END_DATE=2024-12-31
# # or
# # TaxSummary_DATE_MACRO=This Fiscal Year-to-date

# # Columns / breakdown (e.g., Months or Total)
# TaxSummary_COLUMNS=Months
# # or: TaxSummary_SUMMARIZE_COLUMN_BY=Months

# # Filters (optional; ignored if not supported by your realm/report)
# TaxSummary_ACCOUNTING_METHOD=Accrual
# TaxSummary_CURRENCY=USD
# TaxSummary_TAX_AGENCY_IDS=1,2
# TaxSummary_TAX_CODE_IDS=5,6
# TaxSummary_TAX_RATE_IDS=100,101
# TaxSummary_DEPARTMENT_IDS=10,11
# TaxSummary_CLASS_IDS=20,21
# TaxSummary_NAME_IDS=300,301

# # Paging & insert behavior
# TaxSummary_STARTPOSITION=1
# TaxSummary_PAGESIZE=2000
# TaxSummary_FORCE_TEXT_INSERT=false
# TaxSummary_COERCE_TABLE_TO_TEXT=true
