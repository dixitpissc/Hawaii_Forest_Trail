...
REPORT_TABLE  = os.getenv("ProfitAndLossDetail_TABLE", "ProfitAndLossDetail")
SAVE_TO_SQL   = os.getenv("ProfitAndLossDetail_SAVE_TO_SQL", "true").lower() == "true"

# Report options (send only when provided)
START_DATE    = os.getenv("ProfitAndLossDetail_START_DATE")               # yyyy-mm-dd
END_DATE      = os.getenv("ProfitAndLossDetail_END_DATE")                 # yyyy-mm-dd
DATE_MACRO    = os.getenv("ProfitAndLossDetail_DATE_MACRO")               # e.g., "This Fiscal Year-to-date"
ACCOUNTING    = os.getenv("ProfitAndLossDetail_ACCOUNTING_METHOD")        # "Cash"|"Accrual"
CURRENCY      = os.getenv("ProfitAndLossDetail_CURRENCY")                 # "USD"|"CAD"|...

# Column breakdowns (QBO may ignore for Detail; safe to pass)
COLUMNS_BY    = os.getenv("ProfitAndLossDetail_COLUMNS")                  # e.g., "Months"
SUMMARIZE_BY  = os.getenv("ProfitAndLossDetail_SUMMARIZE_COLUMN_BY")      # legacy alias

# Common entity filters (QBO ignores unknowns)
ACCOUNT_IDS   = os.getenv("ProfitAndLossDetail_ACCOUNT_IDS")              # "10,12,21"
CUSTOMER_IDS  = os.getenv("ProfitAndLossDetail_CUSTOMER_IDS")             # "123,456"
VENDOR_IDS    = os.getenv("ProfitAndLossDetail_VENDOR_IDS")               # "789,1011"
ITEM_IDS      = os.getenv("ProfitAndLossDetail_ITEM_IDS")                 # "200,201"
NAME_IDS      = os.getenv("ProfitAndLossDetail_NAME_IDS")                 # generic name filter
CLASS_IDS     = os.getenv("ProfitAndLossDetail_CLASS_IDS")                # "10,11"
DEPT_IDS      = os.getenv("ProfitAndLossDetail_DEPARTMENT_IDS")           # "1,2"
TXN_TYPES     = os.getenv("ProfitAndLossDetail_TRANSACTION_TYPES")        # "Invoice,Bill,Check,..."

MINOR_VERSION = os.getenv("QBO_MINOR_VERSION", "75")                      # keep >= 75

# Pagination (many reports ignore; safe to include)
START_POS     = int(os.getenv("ProfitAndLossDetail_STARTPOSITION", "1"))
PAGE_SIZE     = int(os.getenv("ProfitAndLossDetail_PAGESIZE", "1000"))

# Insert behavior
FORCE_TEXT    = os.getenv("ProfitAndLossDetail_FORCE_TEXT_INSERT", "false").lower() == "true"
COERCE_TABLE  = os.getenv("ProfitAndLossDetail_COERCE_TABLE_TO_TEXT", "true").lower() == "true"
...
REPORT_NAME = "ProfitAndLossDetail"
...
def _build_params() -> dict:
    ...
    if TXN_TYPES:    p["transaction_type"]  = TXN_TYPES
...
def _walk_rows_dynamic(rows_container: dict, col_meta: list, current_group_path: list, rows_out: list):
    ...
    for r in rows:
        ...
        if rtype == "Section":
            ...
            # Optional section summary (e.g., per-account subtotal)
...
def _append_top_level_summary(report: dict, col_meta: list, rows_out: list):
    """Capture Rows.Summary (grand totals / Net Income) as RowType='ReportSummary' if present."""
...
def fetch_all_pages_and_flatten() -> pd.DataFrame:
    t1 = ProgressTimer("📥 Fetching Profit and Loss Detail from QBO")
    try:
    ...
    # Optional paging loop pattern (rarely needed; most pulls are one shot)
    # frames = [df]
...
def save_dataframe(df: pd.DataFrame, schema: str, table: str):
    ...
    SAVE_EMPTY = os.getenv("ProfitAndLossDetail_SAVE_EMPTY_SCHEMA", "true").lower() == "true"
    ...
    if df is None or df.empty:
        if SAVE_EMPTY:
            ...
            logger.warning("⚠️ ProfitAndLossDetail: Nothing to save (empty DataFrame).")
        return
...
# Entry point
# ──────────────────────────────────────────────────────────────
def run_profit_and_loss_detail():
    logger.info("🚀 Starting Profit and Loss Detail report pull")
    try:
        ...
        if SAVE_TO_SQL:
            ...
            logger.info("💾 Skipping SQL save (ProfitAndLossDetail_SAVE_TO_SQL=false).")
        logger.info("✅ Done.")
        return df
    except Exception as e:
        msg = str(e)
        logger.error(f"❌ ProfitAndLossDetail report failed: {msg[:2000]}")
        raise
...
#Env credential
# ProfitAndLossDetail_SAVE_TO_SQL=true
# ProfitAndLossDetail_SAVE_EMPTY_SCHEMA=true

# # Date range or macro (P&L Detail is range-based)
# ProfitAndLossDetail_START_DATE=2024-01-01
# ProfitAndLossDetail_END_DATE=2024-12-31
# # or
# # ProfitAndLossDetail_DATE_MACRO=This Fiscal Year-to-date

# # Columns / breakdown (optional; QBO may ignore for Detail)
# ProfitAndLossDetail_COLUMNS=Months
# # ProfitAndLossDetail_SUMMARIZE_COLUMN_BY=Months

# # Filters (optional)
# ProfitAndLossDetail_ACCOUNTING_METHOD=Accrual
# ProfitAndLossDetail_CURRENCY=USD
# ProfitAndLossDetail_ACCOUNT_IDS=10,12,21
# ProfitAndLossDetail_CUSTOMER_IDS=123,456
# ProfitAndLossDetail_VENDOR_IDS=789,1011
# ProfitAndLossDetail_ITEM_IDS=200,201
# ProfitAndLossDetail_NAME_IDS=888,889
# ProfitAndLossDetail_CLASS_IDS=10,11
# ProfitAndLossDetail_DEPARTMENT_IDS=1,2
# ProfitAndLossDetail_TRANSACTION_TYPES=Invoice,Bill,SalesReceipt,Check,JournalEntry

# # Paging & insert behavior
# ProfitAndLossDetail_STARTPOSITION=1
# ProfitAndLossDetail_PAGESIZE=2000
# ProfitAndLossDetail_FORCE_TEXT_INSERT=false
# ProfitAndLossDetail_COERCE_TABLE_TO_TEXT=true
