...
def fetch_qbo_account_by_name(account_name):
    """Fetch QBO account by Name, return Id and SyncToken (left for parent lookup if needed)"""
...
def prepare_mapping():
    logger.info("🚀 Preparing Map_Account with latest QBO data (fetching by Target_Id)...")
    ...
    # Assume Target_Id column holds the QBO account id (if not present we default to Source_Id)
    df_accounts["Target_Id"] = df_accounts.get("Target_Id", df_accounts["Source_Id"])
    ...
    for idx, row in df_accounts.iterrows():
        # Fetch SyncToken for the account using Target_Id (do not fetch by name)
        qbo_id, sync_token = fetch_qbo_account_by_id(row["Target_Id"])
        df_accounts.at[idx, "SyncToken"] = sync_token
        df_accounts.at[idx, "Target_Id"] = qbo_id

        # Fetch ParentRef.value from QBO using ParentRef.Name (unchanged)
...
def update_qbo_accounts():
    ...
    for idx, row in df_mapping.iterrows():
        ...
        for attempt in range(1, max_retries + 1):
            try:
                logger.info(f"Attempt {attempt}/{max_retries} updating {row['Source_Id']} ({row.get('Name')}) with SyncToken={payload.get('SyncToken')}")
                ...
                if resp.status_code in (200, 201):
                    ...
                    logger.info(f"✅ Updated account {row['Source_Id']} ({row.get('Name')}) on attempt {attempt}")
                    success = True
                    break
                ...
                logger.warning(f"❌ Failed updating {row['Source_Id']} ({row.get('Name')}): HTTP {resp.status_code}: {last_response_text}")
                break
