...
# ---------------------- Config -------------------------
CSV_PATH = "Deposit_Department_Update.csv"
...
log_filename = f"department_update_deposit_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
LOG_FILE_PATH = os.path.join(LOG_DIR, log_filename)

logger = logging.getLogger("department_update_deposit")
...
# In-memory structured logs to be written at the end as deposit_log
deposit_log_entries = []
deposit_log_lock = threading.Lock()

# Wrapper functions that log to both logger and in-memory list
def log_info(msg):
    ts = datetime.now().isoformat()
    logger.info(msg)
    with deposit_log_lock:
        deposit_log_entries.append({"time": ts, "level": "INFO", "message": msg})

def log_warn(msg):
    ts = datetime.now().isoformat()
    logger.warning(msg)
    with deposit_log_lock:
        deposit_log_entries.append({"time": ts, "level": "WARN", "message": msg})

def log_err(msg):
    ts = datetime.now().isoformat()
    logger.error(msg)
    with deposit_log_lock:
        deposit_log_entries.append({"time": ts, "level": "ERROR", "message": msg})
...
# ---------------------- Deposit Logic ------------------
def get_deposit(session, base, realm, deposit_id, access_token):
    url = f"{base}/v3/company/{realm}/deposit/{deposit_id}"
    r = session.get(
        url,
    ...
    return handle_response(r, "Deposit")

def update_deposit(session, base, realm, body, access_token):
    url = f"{base}/v3/company/{realm}/deposit"
    r = session.post(
        url,
        headers=qbo_headers(access_token),
...
def set_department_header_only(deposit: Dict[str, Any], dept_id: str) -> Dict[str, Any]:
    dep_obj = json.loads(json.dumps(deposit))
    dep_obj["sparse"] = False

    if not dep_obj.get("Id") or dep_obj.get("SyncToken") is None:
        raise ValueError("Deposit missing Id or SyncToken")

    dep_obj["DepartmentRef"] = {"value": str(dept_id)}
...
# ---------------------- Worker -------------------------
def process_row(limiter, session, base, realm, tokens, refresh_args, lock, deposit_id, dept):
    try:
        deposit = with_retries(
            limiter, session, get_deposit,
            tokens, refresh_args, lock,
            base, realm, deposit_id
        )

        deposit = set_department_header_only(deposit, dept)

        updated = with_retries(
            limiter, session, update_deposit,
            tokens, refresh_args, lock,
            base, realm, deposit
        )

        return deposit_id, True, updated.get("SyncToken")
    except Exception as e:
        return deposit_id, False, str(e)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=CONCURRENCY) as exe:
        ...
        for f in as_completed(futures):
            deposit_id, ok, msg = f.result()
            if ok:
                log_info(f"✅ Deposit {deposit_id} updated (New SyncToken {msg})")
                with deposit_log_lock:
                    deposit_log_entries.append({"time": datetime.now().isoformat(), "deposit_id": deposit_id, "ok": True, "message": f"New SyncToken {msg}"})
            else:
                log_err(f"❌ Deposit {deposit_id} failed: {msg}")
                with deposit_log_lock:
                    deposit_log_entries.append({"time": datetime.now().isoformat(), "deposit_id": deposit_id, "ok": False, "message": str(msg)})

    # Write structured deposit_log JSON file with timestamped name
    try:
        deposit_log_path = os.path.join(LOG_DIR, f"deposit_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
        with open(deposit_log_path, "w", encoding="utf-8") as fh:
            json.dump(deposit_log_entries, fh, indent=2, ensure_ascii=False)
        logger.info(f"✅ deposit_log written to {deposit_log_path}")
    except Exception:
        logger.exception("❌ Failed to write deposit_log")
