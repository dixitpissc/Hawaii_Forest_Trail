...
# ---------------------- Config -------------------------
CSV_PATH = "VendorCredit_Department_Update.csv"
...
log_filename = f"department_update_vendorcredit_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
LOG_FILE_PATH = os.path.join(LOG_DIR, log_filename)

logger = logging.getLogger("department_update_vendorcredit")
...
# In-memory structured logs to be written at the end as vendorcredit_log
vendorcredit_log_entries = []
vendorcredit_log_lock = threading.Lock()

# Wrapper functions that log to both logger and in-memory list
def log_info(msg):
    ts = datetime.now().isoformat()
    logger.info(msg)
    with vendorcredit_log_lock:
        vendorcredit_log_entries.append({"time": ts, "level": "INFO", "message": msg})

def log_warn(msg):
    ts = datetime.now().isoformat()
    logger.warning(msg)
    with vendorcredit_log_lock:
        vendorcredit_log_entries.append({"time": ts, "level": "WARN", "message": msg})

def log_err(msg):
    ts = datetime.now().isoformat()
    logger.error(msg)
    with vendorcredit_log_lock:
        vendorcredit_log_entries.append({"time": ts, "level": "ERROR", "message": msg})
...
# ---------------------- VendorCredit Logic ---------------------
def get_vendorcredit(session, base, realm, vc_id, access_token):
    url = f"{base}/v3/company/{realm}/vendorcredit/{vc_id}"
    r = session.get(
        url,
    ...
    return handle_response(r, "VendorCredit")

def update_vendorcredit(session, base, realm, body, access_token):
    url = f"{base}/v3/company/{realm}/vendorcredit"
    r = session.post(
        url,
        headers=qbo_headers(access_token),
...
def set_department_header_only(vendorcredit: Dict[str, Any], dept_id: str) -> Dict[str, Any]:
    vc_obj = json.loads(json.dumps(vendorcredit))  # deep copy
    vc_obj["sparse"] = False

    if not vc_obj.get("Id") or vc_obj.get("SyncToken") is None:
        raise ValueError("VendorCredit missing Id or SyncToken")

    vc_obj["DepartmentRef"] = {"value": str(dept_id)}
...
# ---------------------- Worker -------------------------
def process_row(limiter, session, base, realm, tokens, refresh_args, lock, vendorcredit_id, dept):
    try:
        vc = with_retries(
            limiter, session, get_vendorcredit,
            tokens, refresh_args, lock,
            base, realm, vendorcredit_id
        )

        vc = set_department_header_only(vc, dept)

        updated = with_retries(
            limiter, session, update_vendorcredit,
            tokens, refresh_args, lock,
            base, realm, vc
        )

        return vendorcredit_id, True, updated.get("SyncToken")
    except Exception as e:
        return vendorcredit_id, False, str(e)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=CONCURRENCY) as exe:
        ...
        for f in as_completed(futures):
            vendorcredit_id, ok, msg = f.result()
            if ok:
                log_info(f"✅ VendorCredit {vendorcredit_id} updated (New SyncToken {msg})")
                with vendorcredit_log_lock:
                    vendorcredit_log_entries.append({"time": datetime.now().isoformat(), "vendorcredit_id": vendorcredit_id, "ok": True, "message": f"New SyncToken {msg}"})
            else:
                log_err(f"❌ VendorCredit {vendorcredit_id} failed: {msg}")
                with vendorcredit_log_lock:
                    vendorcredit_log_entries.append({"time": datetime.now().isoformat(), "vendorcredit_id": vendorcredit_id, "ok": False, "message": str(msg)})

    # Write structured vendorcredit_log JSON file with timestamped name
    try:
        vendorcredit_log_path = os.path.join(LOG_DIR, f"vendorcredit_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
        with open(vendorcredit_log_path, "w", encoding="utf-8") as fh:
            json.dump(vendorcredit_log_entries, fh, indent=2, ensure_ascii=False)
        logger.info(f"✅ vendorcredit_log written to {vendorcredit_log_path}")
    except Exception:
        logger.exception("❌ Failed to write vendorcredit_log")
