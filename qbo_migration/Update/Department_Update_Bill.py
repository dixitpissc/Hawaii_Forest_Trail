...
# ---------------------- Config -------------------------
CSV_PATH = "Bill_Department_Update.csv"
...
log_filename = f"department_update_bill_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
LOG_FILE_PATH = os.path.join(LOG_DIR, log_filename)

logger = logging.getLogger("department_update_bill")
...
# In-memory structured logs to be written at the end as bill_log
bill_log_entries = []
bill_log_lock = threading.Lock()
...
def log_info(msg):
    ...
    logger.info(msg)
    with bill_log_lock:
        bill_log_entries.append({"time": ts, "level": "INFO", "message": msg})

def log_warn(msg):
    ts = datetime.now().isoformat()
    logger.warning(msg)
    with bill_log_lock:
        bill_log_entries.append({"time": ts, "level": "WARN", "message": msg})

def log_err(msg):
    ts = datetime.now().isoformat()
    logger.error(msg)
    with bill_log_lock:
        bill_log_entries.append({"time": ts, "level": "ERROR", "message": msg})
...
# ---------------------- Bill Logic ---------------------
def get_bill(session, base, realm, bill_id, access_token):
    url = f"{base}/v3/company/{realm}/bill/{bill_id}"
    r = session.get(
        url,
    ...
    return handle_response(r, "Bill")

def update_bill(session, base, realm, body, access_token):
    url = f"{base}/v3/company/{realm}/bill"
    r = session.post(
        url,
        headers=qbo_headers(access_token),
...
def set_department_header_only(bill: Dict[str, Any], dept_id: str) -> Dict[str, Any]:
    bill_obj = json.loads(json.dumps(bill))  # deep copy
    bill_obj["sparse"] = False

    if not bill_obj.get("Id") or bill_obj.get("SyncToken") is None:
        raise ValueError("Bill missing Id or SyncToken")

    bill_obj["DepartmentRef"] = {"value": str(dept_id)}
...
# ---------------------- Worker -------------------------
def process_row(limiter, session, base, realm, tokens, refresh_args, lock, bill_id, dept):
    try:
        bill = with_retries(
            limiter, session, get_bill,
            tokens, refresh_args, lock,
            base, realm, bill_id
        )

        bill = set_department_header_only(bill, dept)

        updated = with_retries(
            limiter, session, update_bill,
            tokens, refresh_args, lock,
            base, realm, bill
        )

        return bill_id, True, updated.get("SyncToken")
    except Exception as e:
        return bill_id, False, str(e)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=CONCURRENCY) as exe:
        ...
        for f in as_completed(futures):
            bill_id, ok, msg = f.result()
            if ok:
                log_info(f"✅ Bill {bill_id} updated (New SyncToken {msg})")
                with bill_log_lock:
                    bill_log_entries.append({"time": datetime.now().isoformat(), "bill_id": bill_id, "ok": True, "message": f"New SyncToken {msg}"})
            else:
                log_err(f"❌ Bill {bill_id} failed: {msg}")
                with bill_log_lock:
                    bill_log_entries.append({"time": datetime.now().isoformat(), "bill_id": bill_id, "ok": False, "message": str(msg)})

    # Write structured bill_log JSON file with timestamped name
    try:
        bill_log_path = os.path.join(LOG_DIR, f"bill_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
        with open(bill_log_path, "w", encoding="utf-8") as fh:
            json.dump(bill_log_entries, fh, indent=2, ensure_ascii=False)
        logger.info(f"✅ bill_log written to {bill_log_path}")
    except Exception:
        logger.exception("❌ Failed to write bill_log")
