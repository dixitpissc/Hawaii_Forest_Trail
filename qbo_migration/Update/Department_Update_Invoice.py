...
# ---------------------- Config -------------------------
CSV_PATH = "Invoice_Department_Update.csv"
...
log_filename = f"department_update_invoice_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
LOG_FILE_PATH = os.path.join(LOG_DIR, log_filename)

logger = logging.getLogger("department_update_invoice")
...
# In-memory structured logs to be written at the end as invoice_log
invoice_log_entries = []
invoice_log_lock = threading.Lock()

# Wrapper functions that log to both logger and in-memory list
def log_info(msg):
    ts = datetime.now().isoformat()
    logger.info(msg)
    with invoice_log_lock:
        invoice_log_entries.append({"time": ts, "level": "INFO", "message": msg})

def log_warn(msg):
    ts = datetime.now().isoformat()
    logger.warning(msg)
    with invoice_log_lock:
        invoice_log_entries.append({"time": ts, "level": "WARN", "message": msg})

def log_err(msg):
    ts = datetime.now().isoformat()
    logger.error(msg)
    with invoice_log_lock:
        invoice_log_entries.append({"time": ts, "level": "ERROR", "message": msg})
...
# ---------------------- Invoice Logic ---------------------
def get_invoice(session, base, realm, invoice_id, access_token):
    url = f"{base}/v3/company/{realm}/invoice/{invoice_id}"
    r = session.get(
        url,
    ...
    return handle_response(r, "Invoice")

def update_invoice(session, base, realm, body, access_token):
...
def set_department_header_only(invoice: Dict[str, Any], dept_id: str) -> Dict[str, Any]:
    invoice_obj = json.loads(json.dumps(invoice))  # deep copy
    invoice_obj["sparse"] = False

    if not invoice_obj.get("Id") or invoice_obj.get("SyncToken") is None:
        raise ValueError("Invoice missing Id or SyncToken")

    invoice_obj["DepartmentRef"] = {"value": str(dept_id)}
...
# ---------------------- Worker -------------------------
def process_row(limiter, session, base, realm, tokens, refresh_args, lock, invoice_id, dept):
    try:
        invoice = with_retries(
            limiter, session, get_invoice,
            tokens, refresh_args, lock,
            base, realm, invoice_id
        )

        invoice = set_department_header_only(invoice, dept)

        updated = with_retries(
            limiter, session, update_invoice,
            tokens, refresh_args, lock,
            base, realm, invoice
        )

        return invoice_id, True, updated.get("SyncToken")
    except Exception as e:
        return invoice_id, False, str(e)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=CONCURRENCY) as exe:
        ...
        for f in as_completed(futures):
            invoice_id, ok, msg = f.result()
            if ok:
                log_info(f"✅ Invoice {invoice_id} updated (New SyncToken {msg})")
                with invoice_log_lock:
                    invoice_log_entries.append({"time": datetime.now().isoformat(), "invoice_id": invoice_id, "ok": True, "message": f"New SyncToken {msg}"})
            else:
                log_err(f"❌ Invoice {invoice_id} failed: {msg}")
                with invoice_log_lock:
                    invoice_log_entries.append({"time": datetime.now().isoformat(), "invoice_id": invoice_id, "ok": False, "message": str(msg)})

    # Write structured invoice_log JSON file with timestamped name
    try:
        invoice_log_path = os.path.join(LOG_DIR, f"invoice_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
        with open(invoice_log_path, "w", encoding="utf-8") as fh:
            json.dump(invoice_log_entries, fh, indent=2, ensure_ascii=False)
        logger.info(f"✅ invoice_log written to {invoice_log_path}")
    except Exception:
        logger.exception("❌ Failed to write invoice_log")
